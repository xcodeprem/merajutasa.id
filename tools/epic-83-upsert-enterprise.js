...
async function main() {
  await ensureDirs();
  const res = { action: 'epic_enterprise_upsert', epic: ISSUE, updated: false, error: null };
  ...
  const out = path.join(ARTIFACTS_DIR, `epic-83-enterprise-upsert-${new Date().toISOString().slice(0,10)}.json`);
  await fs.writeFile(out, stableStringify(addMetadata(res, { generator: 'tools/epic-83-upsert-enterprise.js' })), 'utf8');
...
main().catch(async (err) => {
  try {
    await fs.mkdir(ARTIFACTS_DIR, { recursive: true });
    const p = path.join(ARTIFACTS_DIR, `epic-83-enterprise-upsert-error-${Date.now()}.json`);
    await fs.writeFile(p, stableStringify(addMetadata({ error: err?.message || String(err) }, { generator: 'tools/epic-83-upsert-enterprise.js' })), 'utf8');
