...
import { validateDecFile } from './lib/security-validators.js';
...
async function main(){
  // Validate DEC file path for security
  let validatedDecFile;
  try {
    validatedDecFile = validateDecFile(DEC_FILE);
  } catch (error) {
    console.error(`ERROR: Invalid DEC file path: ${error.message}`);
    process.exit(1);
  }

  if(!(await fileExists(validatedDecFile))){
    console.error(`ERROR: DEC file not found: ${validatedDecFile}`);
    process.exit(1);
  }
  
  const nowIso = NOW_OVERRIDE ? toISO(NOW_OVERRIDE) : new Date().toISOString();
  const decText = await fs.readFile(validatedDecFile, 'utf8');
  let { phase0, phase1, phase2 } = parseScheduleFromDec(decText);

  if(!phase0){
    // derive from git commit timestamp if available (secure execFileSync)
    try {
      const out = execFileSync('git', ['log', '-1', '--format=%cI', '--', validatedDecFile], { 
        encoding: 'utf8',
  ...
  seconds_until_phase2: current==='PHASE_2_DENY' ? 0 : until(phase2),
    ...
    notes: 'Generated by tools/generate-phase-tracker.js (cross-platform, secure)'
  };

  await fs.mkdir('artifacts', { recursive: true });
  await fs.writeFile('artifacts/phase-tracker.json', stableStringify(data));
