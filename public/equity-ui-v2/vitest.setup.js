...
vi.mock('axios', () => {
	const mockGet = vi.fn(async (url) => {
		...
		if (String(url).includes('/kpi/h1')) return { data: { fairness: { pass: true }, equity: { anomalies_count: 0 } } };
		if (String(url).includes('/under-served')) return { data: { total: 0, groups: [] } };
		if (String(url).includes('/kpi/weekly')) return { data: { decision_mix: { counts: { POS: 0, BND: 0, NEG: 0 }, ratios: { POS: 0, BND: 0, NEG: 0 } } } };
		if (String(url).includes('/feedback/monthly')) return { data: { months: [] } };
		if (String(url).includes('/risk/digest')) return { data: { collector: {}, chain: {}, privacy: {} } };
