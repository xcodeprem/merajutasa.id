...
describe('feed adapters', () => {
  it('adaptKPI returns equity and fairness objects', () => {
    ...
    expect(adaptKPI({ equity: { a: 1 }, fairness: { b: 2 } })).toEqual({
      equity: { a: 1 },
      fairness: { b: 2 },
    });
  });
  ...
  it('adaptWeeklyTrends normalizes weeks array and decision mix', () => {
    ...
    const input = {
      weeks: [{ w: 1 }],
      decision_mix: { counts: { POS: 1, NEG: 0, BND: 0 }, ratios: { POS: 1, NEG: 0, BND: 0 } },
    };
  ...
  it('adaptUnderServed ensures numeric total and groups array', () => {
    ...
    expect(adaptUnderServed({ total: 5, groups: [{ id: 'g1' }] })).toEqual({
      total: 5,
      groups: [{ id: 'g1' }],
    });
  });
